...
export class LawoRubyMixerConnection implements MixerConnection {
  ...
  updateAMixState(channelIndex: number, amixOn: boolean) {
    ...
    this.sendOutMessage(protocol.mixerMessage, channelTypeIndex + 1, amixOn, protocol.type)
  }
  ...
  updateInputGain(channelIndex: number, gain: number) {
    ...
    this.sendOutMessage(protocol.mixerMessage, channelTypeIndex + 1, gain, protocol.type)
  }
  updateInputSelector(channelIndex: number, inputSelected: number) {
    ...
    this.sendOutMessage(msg.mixerMessage, channelTypeIndex + 1, msg.value, '')
    return true
  }
